...
const state_1 = require("../utility/state");
...
const handleQuestionResponse = (req, res) => __awaiter(void 0, void 0, void 0, function* () {
    ...
    try {
        ...
        // check chat status
        if (!chatId) {
            chatId = (0, helper_1.generateIdWithPrefix)("chat");
        }
        //manage using state
        const chatState = state_1.chatStateManager.createOrUpdateChat(chatId, {
            assistantID: "",
            vectorStoreID: "",
            threadId: "",
            uploadedDocuments: [],
        ...
        if (receivedFile) {
            ...
            const document = yield openai.files.create({
            ...
            // }
            // add to vectorstore
            yield openai.beta.vectorStores.files.create(chatState.vectorStoreID, {
                file_id: document.id,
            });
        ...
        yield openai.beta.assistants.update(chatState.assistantID, {
            tool_resources: {
                file_search: {
        ...
        if (!chatState.threadId || chatState.threadId === "") {
            ...
            const thread = yield openai.beta.threads.messages.create(chatState.threadId, {
                role: "user",
                content: question,
                attachments: uploadedDocumentsId.map((fileId) => ({
                    file_id: fileId,
            ...
            console.log("thread message: ", (_a = thread.attachments) === null || _a === void 0 ? void 0 : _a.toString);
        }
        // run
        const run = yield openai.beta.threads.runs.createAndPoll(chatState.threadId, {
            assistant_id: assistantID,
        });
        const messages = yield openai.beta.threads.messages.list(chatState.threadId, {
            run_id: run.id,
        });
